---
name: harness-implementation
description: Use this skill when implementing a Code Craft solution after the plan and SOLID review have been approved. It enforces scope discipline, minimal implementation, and verification before and after coding. Tests are written after this skill, with harness-test.
---

# harness-implementation

## Goal

Implement the approved plan with minimal, disciplined code changes and verify the result.

## Rules

- 승인된 책임 범위를 따를 것
- 구현을 작게 유지할 것
- 불필요한 추상화를 추가하지 말 것
- 테스트의 의도를 변경하지 말 것
- 구현 후 빌드 검증은 테스트 필요 여부와 무관하게 항상 실행할 것
- PLAN.md에서 `테스트 필요`로 표시된 책임에 한해 테스트를 실행할 것 (`테스트 불필요` 책임은 빌드 검증만으로 충분)
- 테스트가 실패하면, 코드를 수정하기 전에 어떤 요구사항이 실패했는지 설명할 것
- 이 저장소 외부의 파일을 수정하지 말 것
