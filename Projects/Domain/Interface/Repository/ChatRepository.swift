...
public struct ChatRepository: Sendable {
    public var streamMessage: @Sendable (_ message: String, _ wordID: String) -> AsyncThrowingStream<String, Error> = { _, _ in
        AsyncThrowingStream { $0.finish() }
    }
    /// 로컬 캐시를 먼저 emit하고, 서버 조회가 끝나면 그 결과를 이어서 emit한다(서브플랜 9) —
    /// 단발 반환값으로는 "지금은 이거, 잠시 후엔 저거"를 표현할 수 없어 스트림으로 되어 있다.
    public var fetchHistory: @Sendable (_ wordID: String) -> AsyncThrowingStream<ChatHistory, Error> = { _ in
        AsyncThrowingStream { $0.finish() }
    }
...
extension ChatRepository: UnimplementedTestDependencyKey {}
